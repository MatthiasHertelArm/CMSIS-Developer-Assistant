 ...
 * fails.
 *
 * The word read tries three evaluate requests in turn and keeps the first
 * answer it can parse: a watch expression that dereferences the address, then
 * `x/1xw` and `print/x` through the `-exec` passthrough. cdt-gdb-adapter
 * evaluates `-exec …` as an expression (KB3), so on `gdbtarget` only the first
 * form can answer. A timeout ends the read at once.
 *
...
interface WordQuery {
    ...
    /** The word in the adapter's `result` text, or undefined when there is none. */
    parse(result: string): number | undefined;
}

const WORD_QUERIES: readonly WordQuery[] = [
    { context: 'watch', expressionFor: (address) => `*(unsigned int*)${address}`, parse: leadingInteger },
...
export async function readWordThroughGdb(
    ...
    for (const query of WORD_QUERIES) {
        ...
        try {
            ...
            word = typeof answer === 'string' ? query.parse(answer) : undefined;
        } catch (err) {
