...
suite('DebuggingHandler', () => {
    ...
    suite('execution waits for the armed DAP stopped event', () => {
        test('continue arms the waiter before the request and reports the stop', async () => {
            ...
            const reply = await handlerFor(x).handleContinue();
        ...
        for (const [method, request] of [['handleStepOver', 'stepOver'], ['handleStepInto', 'stepInto'], ['handleStepOut', 'stepOut']] as const) {
            test(`${method} sends ${request} after arming`, async () => {
                ...
                const reply = await handlerFor(x)[method]();
        ...
        test('a timeout pauses the target and says where it is', async () => {
            const x = new ScriptedExecutor();
        ...
        test('pause on a running target waits for the stop', async () => {
            ...
            const reply = await handlerFor(x).handlePause();
        ...
        test('recovery variants: register read failure, no stop, session end, pause failure, current line', async () => {
            ...
            const failedRead = await handlerFor(noRegisters).handleStepOver();
            ...
            assert.ok((await handlerFor(silent).handleStepOver()).includes(
            ...
            assert.ok((await handlerFor(gone).handleStepOver()).includes(
            ...
            assert.ok((await handlerFor(wedged).handleContinue()).endsWith(
            ...
            const located = await handlerFor(withLine).handleContinue();
        ...
        test('the timeout text names the setting, not the wait (KB8)', async () => {
            const x = new ScriptedExecutor();
            x.waiters.push({ kind: 'timeout' }, { kind: 'stopped', reason: 'SIGINT', threadId: 1 });
            const reply = await handlerFor(x, {}, 180).handleContinue({ timeoutMs: 5000 });
        ...
        test('a request that fails after arming is wrapped', async () => {
            ...
            assert.strictEqual(await rejectionOf(handlerFor(x).handleStepOver()), 'Step over failed: Error: adapter gone');
        });
    });
    ...
    suite('fence and state gate', () => {
        test('a failing body answers Error in \'<tool>\'', async () => {
            const x = new ScriptedExecutor();
            x.readMemory = async () => {
                throw new Error('x');
            };
            assert.strictEqual(await handlerFor(x).handleReadMemory({ address: '0x0', length: 4 }), 'Error in \'read_memory\': x');
        });

        test('an overrun answers the handler-level cap and drops the late result', async () => {
            const x = new ScriptedExecutor();
            ...
            try {
                const reply = await handlerFor(x).handleReadMemory({ address: '0x0', length: 4, timeoutMs: 100 });
                assert.ok(reply.startsWith('\'read_memory\' did not complete within 100 ms (handler-level cap).'), reply);
                await delay(300);
        ...
        test('each session state has its own refusal hint', async () => {
            ...
            for (const [state, hint] of Object.entries(hints) as Array<[StatusShape['state'], string]>) {
                ...
                const refusal = `Cannot read memory: session state is '${state}'. ${hint} Use get_session_status for a definitive, never-failing classification.`;
                assert.strictEqual(await handlerFor(x).handleReadMemory({ address: 'zz', length: 4 }), `Error in 'read_memory': ${refusal}`);
                assert.deepStrictEqual(x.argsOf('readMemory'), [], 'the gate runs before anything else');
                assert.strictEqual(await rejectionOf(handlerFor(x).handleStepInto()),
                    `Step into failed: Error: Cannot step into: session state is '${state}'. ${hint} Use get_session_status for a definitive, never-failing classification.`);
            }
        });
    });
    ...
    suite('session start, stop, restart and reset', () => {
        test('a second start is refused without touching the configuration manager', async () => {
            ...
            let asked = false;
            const reply = await handlerFor(x, {}, 5, {
                promptForConfiguration: async () => {
                    asked = true;
                    return 'x';
                },
            }).handleStartDebugging({ workingDirectory: '/w', configurationName: 'C' });
            assert.strictEqual(reply, 'A debug session is already active (name=\'?\', state=stopped). Refusing to start a second session. '
        ...
        test('a named configuration starts by name and shows the full state; motion then diffs the breakpoints', async () => {
            ...
            const reply = await h.handleStartDebugging({ workingDirectory: '/w', configurationName: 'CMSIS Debugger: pyOCD' });
            ...
            assert.ok((await h.handleStepOver()).includes('"breakpointsUnchanged": 0'));
            x.state.breakpoints = ['main.c:7'];
            assert.ok((await h.handleStepOver()).includes('"breakpoints": [\n    "main.c:7"\n  ]'));
        });

        test('Default Configuration without a file is refused', async () => {
            const x = new ScriptedExecutor();
            x.session = false;
            const message = await rejectionOf(handlerFor(x).handleStartDebugging({ workingDirectory: '/w', configurationName: 'Default Configuration' }));
            assert.ok(message.startsWith('Error starting debug session: Error: fileFullPath is required when no named configuration is provided.'), message);
        });
        ...
        test('stop without a session, and with one', async () => {
            ...
            idle.session = false;
            assert.strictEqual(await handlerFor(idle).handleStopDebugging(), 'Nothing to stop — no debug session is active.');

            const x = new ScriptedExecutor();
            const reply = await handlerFor(x).handleStopDebugging();
        ...
        test('restart needs a session and waits for it to come back', async () => {
            const idle = new ScriptedExecutor();
            idle.session = false;
            assert.strictEqual(await rejectionOf(handlerFor(idle).handleRestart()),
                'Could not restart the debug session: Error: Nothing to restart — no debug session is active');

            const x = new ScriptedExecutor();
            assert.strictEqual(await handlerFor(x).handleRestart({ timeoutMs: 1 }), 'The debug session has been restarted.');
        ...
        test('reset passes the method default and renders the outcome', async () => {
            const x = new ScriptedExecutor();
            const reply = await handlerFor(x).handleReset({});
            ...
            const idle = new ScriptedExecutor();
            idle.session = false;
            assert.strictEqual(await handlerFor(idle).handleReset({ halt: false }),
                'Error in \'reset\': No active debug session. Start debugging first — reset drives the target through the live probe.');
        });
    });
    ...
    suite('wait_for_stop and pause', () => {
        ...
        test('each wait outcome has its text', async () => {
            const x = new ScriptedExecutor();
            const h = handlerFor(x, fastClock(100));
            const answer = async (outcome: Awaited<ReturnType<IDebuggingExecutor['waitForStop']>>): Promise<string> => {
                x.waitForStop = async () => outcome;
                return h.handleWaitForStop();
            };
            assert.ok((await answer({ kind: 'timeout' })).startsWith('Target did not stop within the timeout — it is still running'));
            assert.ok((await answer({ kind: 'ended' })).startsWith('The debug session ended while waiting for a stop'));
            assert.ok((await answer({ kind: 'already-stopped', reason: null })).startsWith('Target was already stopped (reason: unknown).\n\n{'));
            assert.ok((await answer({ kind: 'stopped', reason: 'breakpoint', threadId: 3 })).startsWith('Target stopped (reason: breakpoint, threadId=3).\n\n{'));
            assert.ok((await answer({ kind: 'stopped', reason: 'step', threadId: null })).startsWith('Target stopped (reason: step).'));

            const idle = new ScriptedExecutor();
            idle.session = false;
            assert.strictEqual(await handlerFor(idle).handleWaitForStop(),
                'Error in \'wait_for_stop\': No active debug session. Start debugging first — wait_for_stop waits on a live session.');
        });
        ...
        test('pause answers by session state', async () => {
            ...
            assert.strictEqual(await h.handlePause(), 'Target is already stopped (reason: breakpoint). No pause needed — proceed with inspection tools.');
            x.sessionStatus = status('stopped');
            assert.ok((await h.handlePause()).startsWith('Target is already stopped (reason: n/a).'));
            x.sessionStatus = status('unresponsive');
            assert.strictEqual(await h.handlePause(), 'Cannot pause: probe/GDB server is unresponsive. Call check_target_connection or restart_debugging.');
            x.sessionStatus = status('initializing');
            assert.strictEqual(await h.handlePause(), 'Cannot pause yet: session is still initializing. Wait briefly and retry.');
            x.sessionStatus = status('no-session');
            assert.strictEqual(await rejectionOf(h.handlePause()), 'No active debug session.');
            x.sessionStatus = status('running');
            x.waiters.push({ kind: 'timeout' });
            assert.ok((await h.handlePause({ timeoutMs: 700 })).startsWith('Pause requested but target did not stop within the timeout.'));
            assert.deepStrictEqual(x.argsOf('pause'), [[700]]);
            x.waiters.push({ kind: 'ended' });
            assert.strictEqual(await h.handlePause(), 'Pause requested but the debug session ended. The target may have crashed.');
            x.session = false;
            assert.strictEqual(await rejectionOf(h.handlePause()), 'No active debug session. Start debugging first.');
        });
    });
    ...
    suite('breakpoints and logpoints', () => {
        ...
        test('line numbers are checked and a location is required', async () => {
            const x = new ScriptedExecutor();
            assert.strictEqual(await rejectionOf(handlerFor(x).handleAddBreakpoint({ fileFullPath: '/w/main.c', line: 0 })),
        ...
        test('without a session the breakpoint only goes to the model', async () => {
            const x = new ScriptedExecutor();
            x.session = false;
            const reply = await handlerFor(x).handleAddBreakpoint({ fileFullPath: '/w/main.c', line: 10, condition: 'i > 3' });
        ...
        test('the GDB reply decides how the result reads', async () => {
            ...
            assert.strictEqual(await h.handleAddBreakpoint({ fileFullPath: '/w/main.c', line: 10 }),
            ...
            assert.ok((await h.handleAddBreakpoint({ fileFullPath: '/w/main.c', line: 10 })).startsWith(
        ...
        test('a rejection wins over a binding', async () => {
            ...
            const reply = await handlerFor(x).handleAddBreakpoint({ fileFullPath: file, lineContent: 'int' });
        ...
        test('the deprecated lineContent form sets every matching line', async () => {
            ...
            const x = new ScriptedExecutor();
            const reply = await handlerFor(x).handleAddBreakpoint({ fileFullPath: file, lineContent: 'return;' });
        ...
        test('a logpoint without a session only goes to the model', async () => {
            const x = new ScriptedExecutor();
            x.session = false;
            const reply = await handlerFor(x).handleAddLogpoint({ fileFullPath: '/w/main.c', line: 12, logMessage: 'n={n}', condition: 'n > 1' });
        ...
        test('a logpoint condition goes to the dprintf GDB numbered', async () => {
            ...
            const reply = await handlerFor(x).handleAddLogpoint({ fileFullPath: '/w/main.c', line: 12, logMessage: 'n={n:%u}', condition: 'n > 5' });
        ...
        test('a logpoint condition without a number, and a rejected dprintf', async () => {
            const x = new ScriptedExecutor();
            x.gdbReply = 'Error: could not evaluate expression';
            const unnumbered = await handlerFor(x).handleAddLogpoint({ fileFullPath: '/w/main.c', line: 3, logMessage: 'tick', condition: 'c % 2 == 0' });
            ...
            const rejected = await handlerFor(x).handleAddLogpoint({ fileFullPath: '/w/main.c', line: 3, logMessage: '{q}', condition: 'q' });
        ...
        test('remove clears GDB only for a breakpoint the model has', async () => {
            const x = new ScriptedExecutor();
            const h = handlerFor(x);
            assert.strictEqual(await h.handleRemoveBreakpoint({ fileFullPath: '/w/main.c', line: 5 }),
        ...
        test('clear all counts the model and notes the GDB delete', async () => {
            const idle = new ScriptedExecutor();
            idle.session = false;
            assert.strictEqual(await handlerFor(idle).handleClearAllBreakpoints(), 'Nothing to clear — no breakpoints are set.');
            idle.breakpoints = [sourceBreakpoint('/w/a.c', 1), sourceBreakpoint('/w/b.c', 2)];
            assert.strictEqual(await handlerFor(idle).handleClearAllBreakpoints(), 'Cleared 2 breakpoint(s) from the model.');

            const live = new ScriptedExecutor();
            assert.strictEqual(await handlerFor(live).handleClearAllBreakpoints(),
        ...
        test('the list numbers every breakpoint, printing source and function kinds', async () => {
            const x = new ScriptedExecutor();
            assert.strictEqual(await handlerFor(x).handleListBreakpoints(), 'The breakpoint list is empty.');
            x.breakpoints = [
            ...
            assert.strictEqual(await handlerFor(x).handleListBreakpoints(),
    ...
    suite('variables and expressions', () => {
        ...
        test('a missing global scope widens to all', async () => {
            ...
            const reply = await handlerFor(x, focusedOn(7)).handleGetVariables({ scope: 'global', timeoutMs: 900 });
            ...
            const none = await handlerFor(x, focusedOn(7)).handleGetVariables({ variableNames: ['nope', 'other'] });
        ...
        test('the variable tools need a focused frame and scopes', async () => {
            const x = new ScriptedExecutor();
            assert.strictEqual(await handlerFor(x, focusedOn(undefined)).handleListVariableNames({}),
                'Error in \'list_variable_names\': There is no active stack frame. Pause execution at a breakpoint first.');
            assert.strictEqual(await handlerFor(x, focusedOn(0)).handleGetVariables({}),
            ...
            assert.ok((await handlerFor(x, focusedOn(3)).handleListVariableNames({ scope: 'local' })).includes('  count: int\n'));
        });
        ...
        test('evaluate redacts by name, leaves -exec alone and shows the type', async () => {
            ...
            assert.strictEqual(await h.handleEvaluateExpression({ expression: 'apiKey' }),
                `Evaluated: apiKey\nResult: <redacted: possible secret>\nType: char *\n\n${REDACTION_NOTICE}`);
            assert.strictEqual(await h.handleEvaluateExpression({ expression: '  -exec info registers' }),
                'Evaluated:   -exec info registers\nResult: ghp_abcdefghijklmnopqrstuv0123');
            x.evaluateExpression = async () => ({ result: undefined });
            assert.strictEqual(await h.handleEvaluateExpression({ expression: 'x' }),
                'Error in \'evaluate_expression\': The debug adapter returned no result for this expression.');
            x.evaluateExpression = async () => ({ result: 42, type: 'int' });
            assert.strictEqual(await h.handleEvaluateExpression({ expression: 'x' }), 'Evaluated: x\nResult: 42\nType: int');
        });

        test('frame variables use the given frame', async () => {
            const x = new ScriptedExecutor();
            const h = handlerFor(x, focusedOn(undefined));
            assert.strictEqual(await h.handleGetFrameVariables({ frameId: 12 }), 'No variable scopes available for frameId=12.');
            x.getVariablesForFrame = async () => ({ scopes: localScope });
            assert.strictEqual(await h.handleGetFrameVariables({ frameId: 12, variableNames: ['zz'] }),
                'None of the requested variables are in scope at frameId=12: zz.');
            assert.ok((await h.handleGetFrameVariables({ frameId: 12 })).startsWith('Variables for frameId=12:\n==========\n\nLocal:\n  count: 3 (int)'));
        });
    });
    ...
    suite('raw target reads', () => {
        test('memory dump: header, hex rows and ASCII block', async () => {
            ...
            const reply = await handlerFor(x).handleReadMemory({ address: '20000000', length: 20, format: 'both' });
            ...
            assert.strictEqual(await handlerFor(x).handleReadMemory({ address: '0x20000000', length: 20, format: 'hex' }),
                'Memory at 0x20000000 (20 bytes):\n\nHex:\n  0x20000000: 48 65 6c 6c\n');
            assert.ok((await handlerFor(x).handleReadMemory({ address: 'zz', length: 4 })).startsWith('Error in \'read_memory\': '));
        });

        test('a long ASCII block breaks after every 64 characters', async () => {
            const x = new ScriptedExecutor();
            x.readMemory = async () => Buffer.alloc(70, 0x41);
            assert.strictEqual(await handlerFor(x).handleReadMemory({ address: '0x0', length: 70, format: 'ascii' }),
        ...
        test('core registers are normalised and laid out in fixed cells', async () => {
            ...
            const reply = await handlerFor(x).handleReadCoreRegisters();
        ...
        test('cycle counter, present or not', async () => {
            ...
            assert.strictEqual(await handlerFor(x).handleReadCycleCounter(),
            ...
            assert.strictEqual(await handlerFor(x).handleReadCycleCounter(), 'DWT cycle counter: 255 (0x000000ff)\n'
        ...
        test('peripheral registers and fault info pass the executor text through', async () => {
            const x = new ScriptedExecutor();
            assert.strictEqual(await handlerFor(x).handleReadPeripheralRegister({ peripheral: 'RCC' }), 'peripheral text');
            assert.strictEqual(await handlerFor(x).handleGetFaultInfo(), 'fault text');
        });
    });
    ...
    suite('fault diagnosis and SVD lookups', () => {
        test('diagnose_fault degrades section by section', async () => {
            ...
            const reply = await handlerFor(x).handleDiagnoseFault();
            ...
            const noStack = await handlerFor(y).handleDiagnoseFault({ levels: 5, timeoutMs: 4000 });
        ...
        test('an EXC_RETURN in LR reads the stacked frame', async () => {
            ...
            assert.ok((await handlerFor(x).handleDiagnoseFault()).includes('Exception frame: short read at MSP'));
            x.registers = { lr: '0xfffffffd', msp: '0x20001000' };
            assert.ok((await handlerFor(x).handleDiagnoseFault()).includes('Exception frame: PSP unavailable — read_core_registers'));
        });

        test('lookup_peripheral: address first, then name, then the list', async () => {
            const h = handlerFor(new ScriptedExecutor());
            const byAddress = await h.handleLookupPeripheral({ svdFile: FIXTURE_SVD, address: '0x40023800', name: 'GPIOA' });
            assert.ok(byAddress.startsWith('0x40023800 = RCC.CR'), byAddress);
            assert.strictEqual(await h.handleLookupPeripheral({ svdFile: FIXTURE_SVD, address: 'zz' }),
                '\'zz\' is not an address — pass hex like 0x40005400 or a decimal number.');
            assert.strictEqual(await h.handleLookupPeripheral({ svdFile: FIXTURE_SVD, name: 'RCX' }),
                'Peripheral \'RCX\' is not in the SVD of TESTDEVICE. Did you mean: RCC?\nCall lookup_peripheral without name for the full list.');
            assert.ok((await h.handleLookupPeripheral({ svdFile: FIXTURE_SVD, name: 'gpioa' })).startsWith('=== GPIOA @ 0x40020000 ==='));
            assert.ok((await h.handleLookupPeripheral({ svdFile: FIXTURE_SVD })).startsWith('TESTDEVICE: 5 peripherals'));
        });

        test('lookup_register names what is missing', async () => {
            const h = handlerFor(new ScriptedExecutor());
            assert.strictEqual(await h.handleLookupRegister({ svdFile: FIXTURE_SVD, peripheral: 'RCC', register: 'CRX' }),
                'Register \'CRX\' is not in RCC. Did you mean: CR?\nCall lookup_peripheral { name: \'RCC\' } for its register map.');
            assert.ok((await h.handleLookupRegister({ svdFile: FIXTURE_SVD, peripheral: 'rcc', register: 'cr' })).startsWith('=== RCC.CR @ 0x40023800'));
            const missing = await h.handleLookupRegister({ svdFile: '/nonexistent/device.svd', peripheral: 'RCC', register: 'CR' });
    ...
    suite('stack, threads, status, device and connection', () => {
        test('call stack: default cut, explicit depth, and frame details', async () => {
            ...
            const cut = (await handlerFor(x).handleGetCallStack({ threadId: 2 })).split('\n');
            ...
            const all = (await handlerFor(x).handleGetCallStack({ levels: 30 })).split('\n');
            assert.strictEqual(all.length, 1 + 25 + 2);

            x.getCallStack = async () => [{ name: 'Reset_Handler' }];
            assert.strictEqual(await handlerFor(x).handleGetCallStack({}),
                'Call stack (1 frame):\n  # 0  Reset_Handler  @ <no source>:?\n\n'
                + 'Tip: use get_frame_variables with a frameId to inspect variables of a specific frame.');
            x.getCallStack = async () => [];
            assert.strictEqual(await handlerFor(x).handleGetCallStack({}), 'No stack frames available.');
        });
        ...
        test('threads: top frames and the cut at 32', async () => {
            ...
            const lines = (await handlerFor(x).handleGetThreads()).split('\n');
            ...
            x.getThreads = async () => [];
            assert.strictEqual(await handlerFor(x).handleGetThreads(), 'No threads reported by the debug adapter.');
        });
        ...
        test('check_target_connection turns a hardware timeout into text', async () => {
            ...
            assert.strictEqual(await handlerFor(x).handleCheckTargetConnection(), `check_target_connection: ${timeout.message}`);
            x.checkTargetConnection = async () => {
            ...
            assert.strictEqual(await handlerFor(x).handleCheckTargetConnection(), 'No active debug session.');
        });
        ...
        test('session status lists what is known and ends with a hint for the state', async () => {
            ...
            assert.strictEqual(await handlerFor(x).handleGetSessionStatus(), 'State: stopped\nSession: S\nType: gdbtarget\nConfiguration: C\n'
            ...
            for (const [state, hint] of hints) {
                ...
                const text = await handlerFor(x).handleGetSessionStatus();
            ...
            const empty = await handlerFor(x).handleGetSessionStatus();
        ...
        test('device info adds the active CMSIS target', async () => {
            ...
            try {
                assert.strictEqual(await handlerFor(x).handleGetDeviceInfo(),
                    '=== Debug Session Info ===\n  Session name: S\n  CMSIS target: HP@debug');
                answer = () => {
                    throw new Error('not ready');
                };
                assert.strictEqual(await handlerFor(x).handleGetDeviceInfo(), '=== Debug Session Info ===\n  Session name: S\n');
            } finally {
                registration.dispose();
            }
            x.session = false;
            assert.strictEqual(await rejectionOf(handlerFor(x).handleGetDeviceInfo()),
                'Error getting device info: Error: No active debug session. Start debugging first.');
        });
    });
    ...
    suite('cmsis_action', () => {
        ...
        test('refusals come before any CMSIS command', async () => {
            ...
            assert.strictEqual(await handlerFor(x).handleCmsisCommand({ action: 'load_and_debug' }),
            ...
            assert.strictEqual(await handlerFor(x).handleCmsisCommand({ action: 'detach', target: '@x' }),
            ...
            assert.strictEqual(await handlerFor(x).handleCmsisCommand({ action: 'detach', target: 'HP' }),
                'CMSIS \'detach\' not attempted: the active target is \'HE\' and switching to \'HP\' re-activates the solution, '
                + 'which is not done under a live debug session. Call stop_debugging first, then repeat this call.');
        });
        ...
        test('an unknown action is noticed only after the solution checks (KB17)', async () => {
            ...
            assert.strictEqual(await handlerFor(x).handleCmsisCommand({ action }), 'Error in \'cmsis_action\': Unknown CMSIS action: reboot');
            assert.deepStrictEqual(asked, ['solution', 'target']);
        });

        test('no active solution is answered before anything runs', async () => {
            const x = new ScriptedExecutor();
            const reply = await handlerFor(x).handleCmsisCommand({ action: 'stop_run' });
            assert.ok(reply.startsWith('CMSIS \'stop_run\' not attempted: the CMSIS Solution extension reports no active solution '
                + 'in this VS Code window (cmsis-csolution.getSolutionFile → query failed: '), reply);
            assert.ok(reply.includes('(serverVersion=9.8.7)'), reply);
        });
        ...
        test('a failing command within the kick-off is returned through the fence; a late one is dropped', async () => {
            ...
            const noSolution = await handlerFor(x).handleCmsisCommand({ action: 'detach' });
            assert.ok(noSolution.startsWith('Error in \'cmsis_action\': CMSIS \'detach\' failed: no active CMSIS solution. '), noSolution);
            ...
            const other = await handlerFor(x).handleCmsisCommand({ action: 'detach' });
            assert.match(other, /^Error in 'cmsis_action': CMSIS command 'cmsis-csolution.cmsisDetachDebugger' failed: .*boom.*\. Ensure the CMSIS Solution extension is installed and a solution context is active\./s);
            registrations?.dispose();

            solutionOn('HE', {
                'cmsis-csolution.cmsisDetachDebugger': () => new Promise((_, fail) => setTimeout(() => fail(new Error('late')), 300)),
            });
            assert.strictEqual(await handlerFor(x, fastClock(100)).handleCmsisCommand({ action: 'detach' }),
        ...
        suite('build-class actions follow the task', () => {
            ...
            const build = async (steps: () => void, timeoutMs?: number): Promise<string> => {
            ...
            test('exit codes 0, 2 and none', async () => {
                ...
                assert.strictEqual(await build(runTask(cbuild, cbuild, 0)),
                ...
                assert.strictEqual(await build(runTask(cbuild, cbuild, 2)),
                ...
                assert.strictEqual(await build(runTask(cbuild, cbuild, undefined)),
            ...
            test('a task named in capitals from a shell never matches (KB5)', async () => {
                ...
                assert.ok((await build(runTask(shell, shell, 0), 8000)).startsWith(
                    'CMSIS \'build\' on HE issued via \'cmsis-csolution.build\', but no cbuild/flash task ran within the wait window.'));
            });

            test('a task that starts and does not end is reported as still running', async () => {
                const cbuild = task('cbuild demo.csolution.yml');
                assert.ok((await build(runTask(cbuild, undefined), 8000)).startsWith(
            ...
            test('the end of an unrelated execution settles the call (KB5)', async () => {
                assert.ok((await build(runTask(undefined, task('cbuild other'), 0))).startsWith(
                    '✅ CMSIS \'build\' succeeded on HE (task \'cbuild other\' exited 0).'));
            });
        });

        test('attach waits for the session and probes it twice', async () => {
            const attach = async (threadCounts: number[] | undefined): Promise<string> => {
                const x = new ScriptedExecutor();
                x.session = false;
            ...
            const survived = await attach([1, 1]);
            ...
            const lost = await attach([1, 0]);
            assert.ok(lost.startsWith('CMSIS \'attach\' on HE started a debug session but it did NOT survive the initial connect — '
                + 't+3s: 1 thread(s), then t+6s: session object present but 0 threads — adapter is not connected to a target.'), lost);
            const never = await attach(undefined);
    ...
    suite('flash', () => {
        ...
        test('refused under a live session', async () => {
            assert.ok((await handlerFor(new ScriptedExecutor()).handleFlash({})).startsWith(
        ...
        test('the cbuild-run file is resolved or its absence explained', async () => {
            ...
            assert.strictEqual(await withFolder.handleFlash({ cbuildRunFile: 'out/x.cbuild-run.yml' }),
            ...
            assert.strictEqual(await handlerFor(x, { workspaceFolders: () => [] }).handleFlash({}),
            ...
            assert.strictEqual(await twoMatches.handleFlash({}),
            ...
            assert.strictEqual(await noMatch.handleFlash({}),
