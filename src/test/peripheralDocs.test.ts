...
suite('get_peripheral_docs', () => {
    ...
    test('USART1: chapters by type alias, register pages by token, RCC bits, vector, errata state', async () => {
        const text = textOf(await handler.handleGetPeripheralDocs({ peripheral: 'USART1', doc: 'test-rm' }));
    ...
    test('a core peripheral matches a section title when no chapter names it', async () => {
        const text = textOf(await handler.handleGetPeripheralDocs({ peripheral: 'SysTick', doc: 'test-rm', aspects: ['chapters'] }));
    ...
    test('TIM2: instance-level chapter, TIMx register token, cluster register, derived USART2 inherits registers', async () => {
        const tim = textOf(await handler.handleGetPeripheralDocs({ peripheral: 'tim2', doc: 'test-rm' }));
        ...
        const usart2 = textOf(await handler.handleGetPeripheralDocs({ peripheral: 'USART2', doc: 'test-rm', aspects: ['chapters', 'registers'] }));
    ...
    test('a type name lists the instances; unknown names list the groups; no SVD is reported', async () => {
        const uart = textOf(await handler.handleGetPeripheralDocs({ peripheral: 'UART', doc: 'test-rm' }));
        ...
        const nope = textOf(await handler.handleGetPeripheralDocs({ peripheral: 'XYZ', doc: 'test-rm' }));
        ...
        const noSvd = textOf(await handler.handleGetPeripheralDocs({ peripheral: 'USART1', device: 'STM32F756VGTx' }));
        ...
        const systick = textOf(await handler.handleGetPeripheralDocs({ peripheral: 'SysTick', device: 'STM32F756VGTx', doc: 'test-rm', aspects: ['chapters', 'registers'] }));
        ...
        assert.match(textOf(await handler.handleGetPeripheralDocs({ peripheral: '' })), /peripheral is required/);
    });
});
